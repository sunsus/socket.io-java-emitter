...
public class Emitter {
  ...
  */
  private static enum Flag {
	...
	private Emitter(JedisPool redis, HashMap<String,String> opts) {
		if(opts.containsKey("key")) {
			this.key = opts.get("key").toString() + "#emitter";
		} else {
			this.key = "socket.io#emitter";
		}
		
		if(redis == null) {
		    if (!opts.containsKey("host")) throw new Error("Missing redis `host`");
		    if (!opts.containsKey("port")) throw new Error("Missing redis `port`");
    ...
    * @return emitter
    */   
  public static synchronized Emitter getInstance(JedisPool redis, HashMap<String,String> opts) {
      if (instance == null) {
